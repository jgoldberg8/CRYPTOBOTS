...
class TradingSimulator:
    ...
    # Rest of initialization remains the same...
        ...
        try:
            ...
            if not self.global_scaler:  # Only use model scaler if file scaler not loaded
                self.global_scaler = model_data['scaler']
        except Exception as e:
            self.logger.error(f"Error loading peak_before_30 model: {e}")
            raise
            
        # Load peak_market_cap model
        try:
            ...
            if not self.target_scaler:  # Only use model scaler if file scaler not loaded
