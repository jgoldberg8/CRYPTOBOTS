...
class TokenDataCollector:
    def __init__(self, config):
        ...
        self.data_file = os.path.join(log_directory, 'time-data.csv')
    ...
    def _check_record_completion(self, token: dict):
        ...
        if time_since_first >= 3060 and not token.get('recorded', False):
            if token.get('peak_time'):
                time_to_peak = (token['peak_time'] - token['first_trade_time']).total_seconds()
                if time_to_peak <= 1020:
    ...
    def _record_token_data(self, token: dict):
        ...
        try:
            ...
            # Calculate time to peak for use in multiple fields
            time_to_peak = (token['peak_time'] - token['first_trade_time']).total_seconds() \
