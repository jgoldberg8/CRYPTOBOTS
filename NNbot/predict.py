...
from peak_model import PeakMarketCapPredictor, TimeToPeakPredictor, TokenDataset, clean_dataset, add_data_quality_features
...
def evaluate_model_both(peak_market_cap_model_path, time_to_peak_model_path, data_path):
    ...
    plt.plot([true_peak_market_cap.min(), true_peak_market_cap.max()],
             ...
             'r--', lw=2)
    plt.title('One model for each: Peak Market Cap: Predicted vs True')
